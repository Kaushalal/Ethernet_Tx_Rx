...
class emac_tx2rx_ref_model extends uvm_scoreboard;
    ...
    //PORTS FOR SCOREBOARD
	 uvm_analysis_port #(mac_rx_sequence_item ) expected_port3;
	 uvm_analysis_port #(mac_rx_sequence_item ) expected_port4;
	 uvm_analysis_port #(mac_rx_sequence_item ) expected_port5;
   ...
   // int pkt_in_port5[NUM_PORTS];
    ...
    // -----------------------------------------------------------------
    // WRITE FUNCTIONS (With Restored Logging)
    // -----------------------------------------------------------------
    virtual function void write_tx_port3(mac_tx_seq_item  pkt_port3);
        ...
        foreach (tx_mac_port3[i]) begin
            ...
            // Note: Changed .VLAN_ID to .vlan_id to match class definition
            $display("VLAN ID =%h", tx_mac_port3[i].vlan); 
        ...
        forever begin
            ...
            foreach (valid_pkt_etype_port3[i]) begin
                if(valid_pkt_etype_port3[i].payload_q.size() <= 1500) begin
                    `uvm_info("VALID PAYLOAD SIZE", $sformatf("%s: payload size=%d", port_names[PORT3_IDX], valid_pkt_etype_port3[i].payload_q.size()), UVM_DEBUG);
                    ...
                    `uvm_info("INVALID PAYLOAD SIZE", $sformatf("%s: PAYLOAD SIZE=%d - DROPPED", port_names[PORT3_IDX], valid_pkt_etype_port3[i].payload_q.size()), UVM_LOW);
                end
            end
           ...
           $display("port4 valid pkt etype queu size=%d",valid_pkt_etype_port4.size() );
            ...
            foreach (valid_pkt_etype_port4[i]) begin
                if(valid_pkt_etype_port4[i].payload_q.size() <= 1500) begin
                    `uvm_info("VALID PAYLOAD", $sformatf("%s: PAYLOAD SIZE=%d", port_names[PORT4_IDX], valid_pkt_etype_port4[i].payload_q.size()), UVM_DEBUG)
                    valid_mac_pkt_port4.push_back(valid_pkt_etype_port4[i]);
                end else begin
                    payload_drop[PORT4_IDX]++;

                    `uvm_info("INVALID PAYLOAD SIZE", $sformatf("%s: PAYLOAD=%d - DROPPED", port_names[PORT4_IDX], valid_pkt_etype_port4[i].payload_q.size()), UVM_LOW)
                end
            end
        ...
        forever begin
            ...
            foreach (valid_pkt_etype_port5[i]) begin
                if(valid_pkt_etype_port5[i].payload_q.size() <= 1500) begin
                    `uvm_info("VALID PAYLOAD", $sformatf("%s: payload=%d", port_names[PORT5_IDX], valid_pkt_etype_port5[i].payload_q.size()), UVM_HIGH);
                    valid_mac_pkt_port5.push_back(valid_pkt_etype_port5[i]);
                end else begin
                    payload_drop[PORT5_IDX]++;
                    `uvm_info("INVALID PAYLOAD SIZE", $sformatf("%s: PAYLOAD SIZE=%D - DROPPED", port_names[PORT5_IDX], valid_pkt_etype_port5[i].payload_q.size()), UVM_LOW)
                end
            end
    ...
    task vcid_fatch();
       fork
          vcid_fatch_port3();
          // Add ports 4 and 5 here if needed
       join_none
    endtask

    task vcid_fatch_port3();
        mac_rx_sequence_item  expected_pkt;
        ...
        forever begin
            ...
            if (ral.conn_cfg_reg_h[cnn_addr] != null) begin
                ...
                `uvm_info("VCID REG ADDR", $sformatf ("VCID_ADDR=%h", vcid_addr), UVM_LOW) // Restored
                
                if (ral.vcid_reg_h[vcid_addr] != null) begin
                    vcid_field = ral.vcid_reg_h[vcid_addr].vcid;
                    vcid       = vcid_field.get();
                    $display("REF_VCID=%h",vcid); 
                    `uvm_info("VCID", $sformatf ("VCID=%h", vcid), UVM_LOW) // Restored

                    expected_pkt = get_rx_expected(current_pkt, vcid);

                     exp_vcid_q.push_back(expected_pkt.rx_vcid);
    ...
    function mac_rx_sequence_item  get_rx_expected(
        mac_tx_seq_item  valid_expected_pkt,
        bit [7:0] vcid
    );
        mac_rx_sequence_item  expected_pkt;
        expected_pkt = mac_rx_sequence_item ::type_id::create("expected_pkt");

        expected_pkt.rx_SA          = valid_expected_pkt.sa; 
        expected_pkt.rx_DA          = valid_expected_pkt.da;
        expected_pkt.rx_vcid        = vcid;
        expected_pkt.rx_EType       = valid_expected_pkt.Etype; 
        expected_pkt.rx_payload     = valid_expected_pkt.payload_q;
...
function void final_phase(uvm_phase phase);
 ...
 //   $display("No of packet dropped due to incorrect CRC = %0d", total_crc_drop);
   ...
   $display("EXPECTED VCID_Q: %0p", exp_vcid_q);
   endfunction 
endclass
`endif
